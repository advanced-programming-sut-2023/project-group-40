...
public enum Commands{
    ...
    CREATE_UNIT("(?=.* -x (?<x>\\d+))(?=.* -y (?<y>\\d+))(?=.* -t (?<type>((\"[^\"]+\")|(\\S+))))(?=.* -c (?<count>\\d+))^create unit( -[xytc] ((\"[^\"]+\")|(\\S+))){4}$\n", GameMenu.class,"createUnit"),
    ...
    SET_TEXTURE("(?=.* -x (?<x>\\d+))(?=.* -y (?<y>\\d+))(?=.* -t (?<type>((\"[^\"]+\")|(\\S+))))^set texture( -[xytc] ((\"[^\"]+\")|(\\S+))){3}$", EnvironmentMenu.class,"setTexture"),
    ...
    DROP_TOWER("(?=.* -x (?<x>\\d+))(?=.* -y (?<y>\\d+))^drop tower( -[xy] \\S+){2}$",GameMenu.class,"dropTower"),
    DROP_TURRET("(?=.* -x (?<x>\\d+))(?=.* -y (?<y>\\d+))^drop turret( -[xy] \\S+){2}$",GameMenu.class,"dropTurret"),

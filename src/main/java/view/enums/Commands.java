...
public enum Commands{
    ...
    DROP_BUILDING("drop building -x1 (?<x1>\\d+) -y1 (?<y1>\\d+) -x2 (?<x2>\\d+) -y2 (?<y2>\\d+) -type (?<type>\\w+)", GameMenu.class,"dropBuilding"),
    SELECT_BUILDING("select building -x (?<x>\\d+) -y (?<y>\\d+)", GameMenu.class,"selectBuilding"),
    CREATE_UNIT("create unit -x (?<x>\\d+) -y (?<y>\\d+) -t (?<type>\\w+) -c (?<count>\\d)", GameMenu.class,"createUnit"),
